 ...
 *
 * Flow:
 * 1. backend/api/character.ts calls generateCharacterProfile().
 * 2. This service fetches Spotify profile, top tracks, top artists, followed artists, playlists, and audio features.
 * 3. It builds a prompt and sends it to backend/services/gemini.ts.
...
interface CharacterProfilePayload {
  archetype?: string
  stats?: Array<Partial<CharacterStat>>
  verdict?: string
}
...
export interface GeneratedCharacterProfile {
  ...
  sources: {
    ...
    top_artists: TopArtistsResult['artists']
    audio_features: AudioFeaturesResult
    followed_artists: FollowedArtistsResult['artists']
    playlists: PlaylistsResult['playlists']
  }
}

// Keep the time range stable even if the caller sends an invalid value.
export function normalizeTimeRange(timeRange: string) {
...
function buildPrompt(params: {
  ...
  followedArtists: FollowedArtistsResult
  playlists: PlaylistsResult
}) {
  const { timeRange, user, topTracks, topArtists, audioFeatures, followedArtists, playlists } = params

  // Render the top tracks list into prompt-friendly lines.
  const trackLines = topTracks.tracks
    .map(
      (track, index) =>
        `${index + 1}. ${track.name} - ${(track.artists || []).map((artist) => artist.name).join(', ')} (popularity: ${track.popularity ?? 'n/a'})`
    )
    .join('\n')
  ...
  const followedNames = followedArtists.artists.length
    ? followedArtists.artists.slice(0, 10).map((artist) => artist.name).join(', ')
    : 'none'
...
function normalizeStats(stats: Array<Partial<CharacterStat>> | undefined, fallbackStats: CharacterStat[]) {
...
function buildFallbackStats(audioFeatures: AudioFeaturesResult, topArtists: TopArtistsResult): CharacterStat[] {
...
export async function generateCharacterProfile(
  ...
  requestedTimeRange = 'medium_term'
