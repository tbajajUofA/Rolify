...
function handleApiError(res: Response, error: unknown, fallbackMessage: string) {
  console.error(fallbackMessage, error)
  const appError = error as AppError
  return res.status(appError.status || 500).json({ error: appError.message || fallbackMessage })
}
...
router.get('/audio-features', async (req: Request, res: Response) => {
  try {
    ...
    const ids = String(req.query.ids || '')
      ...
      .map((id) => id.trim())
      .filter(Boolean)
    const result = await getAudioFeatures(accessToken, {
      ids,
